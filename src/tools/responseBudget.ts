...
export const maxResponseBytes = () => configured("TG_MAX_RESPONSE_BYTES", 512 * 1024, HARD_MAX_RESPONSE_BYTES);
...
export function budgetResponse(value: unknown, budget = maxResponseBytes()): unknown {
  const bounded = bound(value);
  const serialized = JSON.stringify(bounded);
  if (Buffer.byteLength(serialized, "utf8") <= budget) return bounded;
