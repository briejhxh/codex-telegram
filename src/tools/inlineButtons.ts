...
export function registerClickInlineButton(server: McpServer, { telegram }: ToolContext) {
  server.registerTool("telegram_click_inline_button", { description: "Press a Telegram bot inline callback button by row and column. This sends a callback query and may change bot state. Callback buttons only; payments, URL/login, web-app, game, and password buttons are blocked.", inputSchema: { chat_id: z.number().int(), message_id: z.number().int().positive(), row: z.number().int().min(0), column: z.number().int().min(0) }, annotations: { readOnlyHint: false, destructiveHint: false } }, async ({ chat_id, message_id, row, column }) => safely("telegram_click_inline_button", () => telegram.clickInlineButton(chat_id, message_id, row, column)));
}
